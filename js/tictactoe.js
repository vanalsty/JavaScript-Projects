...
function startGame() {
    ...
    var showPlayer = document.getElementById('showPlayer').innerHTML;
...
function hideGameMsg() {
    clearMsg() //clear text from the message console
...
function saveSettings() {
    ...
    if (p1Selected[p1Index].text == p1Selected[p2Index].text) {
...
//this function retuns the currently assigned avatar for each player
function getAvatars() {
...
//this function returns the active player's avatar
function determineAvatar() {
    // determine the correct avatar to paint for the active player
    var avatarArray = getAvatars(); //returns the array of both player's assigned avatars
...
function avatarPlaced() {
    ...
    activePlayer = showPlayer.innerHTML; //get the curent player from the element
...
//square has already been assinged and if it has not, record the new squares with the assigned avatar.
function recordMoves(square) {
    ...
    verdict = check(info,square); //call function to check if proposed square already occupied
    return verdict;
}
...
function check4Tie() {
    ...
    if (boardState.length >= 9 && check != "That's three in a row, Player 1 wins!" && check != "That's three in a row, Player 2 wins!"){  
        ...
        tieSound(); //play a sound when a ties has been detected
        writeMsg(txt1);
...
function winner(winDetected,winCon) {
    if (winDetected == "win") {
        ...
        startEnabled(btn); //enable the start button since the game is now stopped
        var btn = document.getElementById('btnStop');
        btnDisabled(btn); //disable stop button since the game is now stopped
...
function glowBoard(pos) {
    ...
    for (var i=0;i<squares.length;i++) {
        if (i == index0) {
            ...
            blink();
            winSound();
            setTimeout(function() {bg1.style.backgroundColor = 'rbg(244, 179, 66)';}, 100);
            setTimeout(function() {bg1.style.backgroundColor = 'rbg(244, 238, 66)';}, 200);
            setTimeout(function() {bg1.style.backgroundColor = 'rbg(197, 244, 66)';}, 300);
            setTimeout(function() {bg1.style.backgroundColor = 'rbg(122, 244, 66)';}, 400);
            setTimeout(function() {bg1.style.backgroundColor = 'rbg(66, 244, 235)';}, 500);
            setTimeout(function() {bg1.style.backgroundColor = 'rbg(244, 179, 66)';}, 600);
            setTimeout(function() {bg1.style.backgroundColor = 'rbg(244, 238, 66)';}, 700);
            setTimeout(function() {bg1.style.backgroundColor = 'rbg(197, 244, 66)';}, 800);
            setTimeout(function() {bg1.style.backgroundColor = 'rbg(122, 244, 66)';}, 900);
            setTimeout(function() {bg1.style.backgroundColor = 'rbg(66, 244, 235)';}, 1000);
...
function squareSound() {
    var sound = document.getElementsById("placeAvatar");
    sound.play();
...
function tieSound() {
    var sound = document.getElementsById("tieGame");
...
function checkWinCon1(info,squareArray) {
    var winDetected = "on";
    var winCon1 = [0,1,2];
    //iterate through the growing array during gametime
    //searching for the existence of index 0, index 1, and 
    //index 2 and once they do appear in the array, record
    //their avatars and compare all 3 for win cons
    for (var i in info) {
        if (info[i].charAt(0) =="0") {
            var match0Avatar = info[1].charAt(1); //only interested in recording the avatar
        }
        if (info[i].charAt(0) =="1") {
            var match1Avatar = info[1].charAt(1);
        }
...
if (match0Avatar != undefined && match1Avatar != undefined && match2Avatar != undefined) {
    if (match0Avatar == match1Avatar && match0Avatar == match2Avatar) {
        winDetected == "win"; //this flag will pass when win has been detected
...
function checkWinCon1(info,squareArray) {
    ...
    for (var i in info) {
        ...
        if (info[i].charAt(0) =="4") {
            var match4Avatar = info[1].charAt(1);
        }
...
function square1Animate() {
    var activePlayer = document.getElementById('showPlayer').innerHTML;
    if (activePlayer != "Game Stopped") { //if game has not yet started prevenet avatar placement
        var square = "0"; // indentify the square selected
        ...
        if (verdict == undefined) { //if verdict is empty then the square is unoccupied
            ...
            if (paintAvatar =="O") { //change these all to ternary statements instead
                ...
                //build new array adding the newly selected square and the assinged avatar
            var currentMove = ","+square+paintAvatar;
            recordMove(currentMove);
            checkForWinCon(); //call function to check if current move completes winning condition
